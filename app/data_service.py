...
from app.playoff_context import ACTIVE_PLAYOFF_TEAMS, add_finals_path_context
...
def load_team_stats(season: str, season_type: str, force_refresh: bool = False) -> pd.DataFrame:
    ...
    teams = add_finals_path_context(
        enrich_team_features(fetch_team_stats(season, season_type, force_refresh))
    )
...
def load_team_stats_dataset(
    ...
    teams = add_finals_path_context(enrich_team_features(result.stats))
    active_teams = teams[teams["team_name"].isin(ACTIVE_PLAYOFF_TEAMS)].copy()
